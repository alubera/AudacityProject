...
void AudacityProject::CreateMenusAndCommands()
{
   ...
   c->AddItem(wxT("SortByName"), _("by Name"), FN(OnSortName),
              TracksExistFlag,
              TracksExistFlag);
	c->AddItem(wxT("Sort"), _("by something"), FN(OnNewSort),
				  TracksExistFlag,
				  TracksExistFlag);
         ...
         for (ndx = 0; ndx < (int)arr.GetCount(); ndx++) {
            ...
            //sort by time otherwise
               ...
               ndx+=candidatesLookedAt;
				...
				else if (flags & kAudacityNewSort) {
					printf("%i and %i\n", ((Track*)arr[ndx])->GetKind(), track->GetKind());
					// new sort currently sorts by Track kind...
					if (((Track*)arr[ndx])->GetKind() < track->GetKind()) {
						break;
					}
				}
         }
...
void AudacityProject::OnNewSort()
{
   SortTracks(kAudacityNewSort);

   PushState(_("Tracks sorted by something"), _("New Sort"));
